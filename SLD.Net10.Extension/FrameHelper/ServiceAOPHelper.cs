...
namespace SLD.Net10.Extension.FrameHelper
{
    /// <summary>
    /// 业务服务层全局AOP拦截器
    ...
    /// </summary>
    public class ServiceAOPHelper : IInterceptor
    {
        /// <summary>
        ...
        public void Intercept(IInvocation invocation)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    //_logger.Info(JsonConvert.SerializeObject(aopLog));
        ...
        private void LogEx(Exception ex, ServiceAOPLogInfo aopLog)
        {
            if (ex == null) return;
            // 控制台输出异常日志（生产替换为日志框架）
            Log.Logger.Error($"error!!!:{ex.Message} 完整日志信息：{JsonConvert.SerializeObject(aopLog)}");
        }
